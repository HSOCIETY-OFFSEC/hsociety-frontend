      ...
      >
        <div className="section-container">
          ...
          <div className="overflow-hidden rounded-md border border-border bg-bg-secondary" role="table">
            <div
              className="grid grid-cols-[64px_minmax(0,1.4fr)_minmax(0,0.7fr)_minmax(0,0.7fr)] items-center gap-4 border-t-0 border-border px-4 py-2 font-mono text-[0.6rem] uppercase tracking-[0.14em] text-text-tertiary max-[720px]:grid-cols-[52px_minmax(0,1fr)_minmax(0,0.8fr)] max-[480px]:grid-cols-[44px_minmax(0,1fr)]"
              role="row"
            >
        ...
        <div className="overflow-hidden rounded-md border border-border bg-bg-secondary" role="table">
          ...
          >
            ...
            {top.map((entry, index) => (
              <div
                key={entry.id}
                className={`grid grid-cols-[64px_minmax(0,1.4fr)_minmax(0,0.7fr)_minmax(0,0.7fr)] items-center gap-3 border-t border-border px-4 py-2 text-xs transition-colors hover:bg-bg-tertiary max-[720px]:grid-cols-[52px_minmax(0,1fr)_minmax(0,0.8fr)] max-[480px]:grid-cols-[44px_minmax(0,1fr)] animate-why-in-x ${
                  isVisible ? '' : 'opacity-0 -translate-x-2 motion-reduce:opacity-100 motion-reduce:translate-x-0'
                }`}
                style={isVisible ? { animationDelay: `${(index + 1) * 60}ms` } : undefined}
                role="row"
              >
                <span
                  className={`font-mono text-[0.65rem] text-text-tertiary ${
              ...
              <span className="inline-flex items-center gap-2 text-text-primary" role="cell">
                {(() => {
                  ...
                  return (
                    <ImageWithLoader
                      ...
                      loading="lazy"
                      className="h-5 w-5 rounded-full border border-border bg-bg-tertiary transition-transform duration-200 group-hover:scale-105"
                      imgClassName="h-full w-full object-cover"
                    />
                  );
                })()}
                <span className="flex min-w-0 flex-col gap-1">
                  <span className="truncate text-[0.8rem] font-medium">{entry.name || 'Community member'}</span>
                  <em className="truncate font-mono text-[0.62rem] text-text-tertiary">
                    @{entry.name || entry.handle || 'operator'}
                  </em>
                </span>
              </span>
              <span role="cell" className="font-mono text-[0.7rem] font-medium text-text-secondary max-[480px]:hidden">
                {Number(entry.totalCp || 0).toLocaleString()} CP
              </span>
              <span
                className="w-fit rounded-full border border-border bg-bg-tertiary px-2 py-0.5 font-mono text-[0.62rem] text-text-tertiary transition-colors max-[720px]:hidden"
                role="cell"
              >
                {entry.rank}
              </span>
